...
def build_argparser():
    ...
    p.add_argument(
        "--attn_impl",
        type=str,
        choices=["fa2", "sdpa", "eager"],
        default="sdpa",
        help="Attention backend: flash-attn-2 | SDPA | eager (default sdpa for LOPA stability/speed)",
    )
    ...
    # Gen3 toggle
    p.add_argument("--gen3", type=str2bool, default=True, help="3세대: special-token 압축 제거 + 하위 prefill-layer만 prefill")
    # Gen 단계도 잘라 내는 옵션 (실험용)                    
    p.add_argument("--also_cut_gen", type=str2bool, default=False,
                   help="True면 prefill뿐 아니라 gen 단계도 prefill_layers까지만 실행하고 바로 출력 헤드(lm_head)로 투사")
...
def dc_repeat(dc, B):
    if USE_DYNAMIC_CACHE and isinstance(dc, DynamicCache):
        ...
        for li in range(dc_num_layers(dc)):
            k, v = dc_get_kv(dc, li)
            # Some HF versions keep placeholder layers with (None, None).
            # Skip repeating those; DynamicCache can be partially filled.
        ...
        for (k, v) in dc:
            if k is None or v is None:
                continue
            safe.append((k.repeat(B, 1, 1, 1), v.repeat(B, 1, 1, 1)))
        return safe
...
# Ensure all layers have the same past length for FA2 varlen path
def fa2_equalize_cache(dc, model):
    """Ensure each layer's KV in cache has identical past length and resides on that layer's device.
    This avoids FA2 varlen+past asserts and multi-GPU device mismatches when concatenating past/new KVs.
    """
    try:
        target_len = dc_first_seq_len(dc, 0)
        if target_len <= 0:
            return dc
        cfg = getattr(model, 'config', None)
        n_layers = dc_num_layers(dc)

        # Resolve per-layer device and dtype from the module parameters
        def layer_dev_dtype(li):
            try:
                inner = model.get_base_model() if hasattr(model, 'get_base_model') else model
                lyr = getattr(inner, 'model', inner).layers[li]
                p = next((p for p in lyr.parameters() if p is not None), None)
                if p is not None:
                    return p.device, p.dtype
            except Exception:
                pass
            emb = model.get_input_embeddings().weight
            return emb.device, emb.dtype

        # Head dims
        n_kv = int(getattr(cfg, 'num_key_value_heads', getattr(cfg, 'num_attention_heads', 0)) or 0)
        head_dim = int(getattr(cfg, 'head_dim', (cfg.hidden_size // cfg.num_attention_heads)))

        out = DynamicCache() if (USE_DYNAMIC_CACHE and isinstance(dc, DynamicCache)) else []
        for li in range(n_layers):
            dev, dt = layer_dev_dtype(li)
            k, v = dc_get_kv(dc, li)
            if k is None or v is None:
                k_pad = torch.zeros((1, n_kv, target_len, head_dim), device=dev, dtype=dt)
                v_pad = torch.zeros((1, n_kv, target_len, head_dim), device=dev, dtype=dt)
                dc_update(out, k_pad, v_pad, li)
            else:
                # Move to target layer device/dtype if needed
                if k.device != dev or k.dtype != dt:
                    k = k.to(device=dev, dtype=dt)
                if v.device != dev or v.dtype != dt:
                    v = v.to(device=dev, dtype=dt)
                K = int(k.shape[2])
                if K == target_len:
                    dc_update(out, k, v, li)
                elif K < target_len:
                    pad = target_len - K
                    k_pad = torch.cat([k, torch.zeros((k.shape[0], k.shape[1], pad, k.shape[3]), device=dev, dtype=dt)], dim=2)
                    v_pad = torch.cat([v, torch.zeros((v.shape[0], v.shape[1], pad, v.shape[3]), device=dev, dtype=dt)], dim=2)
                    dc_update(out, k_pad, v_pad, li)
                else:
                    dc_update(out, k[:, :, :target_len, :], v[:, :, :target_len, :], li)
        return out
    except Exception:
        return dc
...
def main():
    ...
    # 4) Special tokens (Gen3에서는 미사용; 호환 위해 옵션 유지)
    support_token = "<|SUPPORT|>"
    doc_sep_token = "<|DOC_SEP|>"
    special_tokens_vocab: List[str] = []
    if not args.gen3:
        ...
        if only_one:
            ...
            latent_markers = build_latent_markers(args.latent_token_num, False)
    ...
    # Select attention backend per flag (LOPA default: sdpa)
    impl = {
    ...
    if lora_enabled:
        ...
        model = get_peft_model(model, lora_config)  # LoRA 파라미터만 학습
    else:
        # Gen3: LoRA 미사용 시 기본적으로 전체 학습
        for p in model.parameters():
            p.requires_grad = True

    # Optimizer는 학습 가능한 모든 파라미터(LoRA 또는 전체)로 구성
    trainable_params = [p for p in model.parameters() if p.requires_grad]
    if len(trainable_params) == 0:
        # Fallback: input embedding만 학습
    ...
    def _build_phase2_prefix_mistral(question: str, special_seq: str):
        # 요청대로 Mistral에서는 [system] → [user: specials+Q]만 사용 (assistant "" 미포함)
        prefix_msgs = [
            {"role": "system", "content": system_prompt},
    ...
    # =============================
    # Gen3 helpers (no special tokens)
    # =============================
    def _prefill_and_build_combined_gen3(question: str, document: str):
        """Prefill sys+doc(+query) only on lower layers and return its cache.
        Also returns the prefix ids/messages used to slice assistant tokens.
        """
        user_txt = f"Document:\n{document}\n\nQuestion: {question}" if include_query_bool else f"Document:\n{document}\n\n"
        msgs_user = [
            {"role": "system", "content": system_prompt},
            {"role": "user",   "content": user_txt},
        ]
        ids_user = tokenizer(tokenizer.apply_chat_template(msgs_user, tokenize=False),
                             add_special_tokens=False, return_tensors="pt").input_ids[0].to(device)
        ...
        if prefill_layers > 0 and prefill_layers < n_layers:
            with torch.no_grad():
                out_low = inner_base.model(
                    input_ids=ids_user.unsqueeze(0),
                    attention_mask=torch.ones(1, ids_user.size(0), device=ids_user.device),
                    use_cache=True,
                    return_dict=True,
                    layers_limit=prefill_layers,
                )
            lower_cache = dc_from_legacy(out_low.past_key_values)
            if args.attn_impl == "fa2":
                lower_cache = fa2_equalize_cache(lower_cache, model)
        else:
            with torch.no_grad():
                out_full = model(
                    ids_user.unsqueeze(0),
                    torch.ones(1, ids_user.size(0), device=ids_user.device),
                    use_cache=True,
                    return_dict=True,
                )
            lower_cache = dc_from_legacy(out_full.past_key_values)
            if args.attn_impl == "fa2":
                lower_cache = fa2_equalize_cache(lower_cache, model)

        prefix_msgs = list(msgs_user)
        # LLaMA(user->assistant header) 정렬만 적용, Mistral은 prefix를 [sys,user]로 유지
        if (not IS_MISTRAL) and header_mode == "assistant":
            msgs_with_hdr = list(msgs_user) + [{"role": "assistant", "content": ""}]
            ids_hdr = tokenizer(tokenizer.apply_chat_template(msgs_with_hdr, tokenize=False),
                               add_special_tokens=False, return_tensors="pt").input_ids[0].to(device)
            if ids_hdr.size(0) > ids_user.size(0):
                hdr_tail = ids_hdr[ids_user.size(0):]
                if prefill_layers > 0 and prefill_layers < n_layers:
                    out_hdr = inner_base.model(
                        input_ids=hdr_tail.unsqueeze(0),
                        attention_mask=torch.ones(1, ids_hdr.size(0), device=ids_hdr.device),
                        past_key_values=lower_cache,
                        use_cache=True,
                        return_dict=True,
                        layers_limit=prefill_layers,
                    )
                    ...
                    out_hdr = model(
                        hdr_tail.unsqueeze(0),
                        torch.ones(1, ids_hdr.size(0), device=ids_hdr.device),
                        past_key_values=lower_cache,
                        use_cache=True,
                        return_dict=True,
                    )
                lower_cache = dc_from_legacy(out_hdr.past_key_values)
                if args.attn_impl == "fa2":
                    lower_cache = fa2_equalize_cache(lower_cache, model)
            prefix_msgs = msgs_with_hdr

        past_len = dc_first_seq_len(lower_cache, default_len=ids_user.size(0))
        pos_base = past_len
        prefix_ids = tokenizer(tokenizer.apply_chat_template(prefix_msgs, tokenize=False),
                               add_special_tokens=False, return_tensors="pt").input_ids[0].to(device)
        return lower_cache, past_len, pos_base, prefix_ids, prefix_msgs

    def forward_gen3_batched_responses(question: str, document: str, responses: List[str]) -> torch.Tensor:
        resp_list = [str(r) for r in (responses or []) if isinstance(r, str) and len(str(r).strip()) > 0]
        if len(resp_list) == 0:
            return torch.tensor(0.0, device=device, requires_grad=True)

        lower_cache, past_len, pos_base, prefix_ids, prefix_msgs = _prefill_and_build_combined_gen3(question, document)
        ...
        for resp in resp_list:
            ...
            if not IS_MISTRAL and header_mode == "assistant" and (not base_msgs or base_msgs[-1].get("role") != "assistant"):
                base_msgs = base_msgs + [{"role":"assistant","content":""}]
            msgs2 = base_msgs + [{"role":"assistant","content": resp}]
            full_ids2 = tokenizer(tokenizer.apply_chat_template(msgs2, tokenize=False),
                                  add_special_tokens=False, return_tensors="pt").input_ids[0].to(device)
        ...
        # Fast batched path (SDPA default): pack assistant continuations and run one forward
        B = len(ass_ids_list)
        ...
        for bi, a_ids in enumerate(ass_ids_list):
            ...
            labels[bi,    :La] = a_ids
        # Build key-padding mask [B, past_len+L]
        attn_mask = torch.cat([
            torch.ones(B, past_len, device=device, dtype=torch.long),
            (input_ids != tokenizer.pad_token_id).long()
        ], dim=1)
        # Repeat cache for batch
        batch_cache = dc_repeat(lower_cache, B)
        gen_layers_limit = int(prefill_layers) if (args.also_cut_gen and int(prefill_layers) > 0 and int(prefill_layers) < int(n_layers)) else None
        out = model(
            input_ids=input_ids,
            ...
            labels=labels,
            return_dict=True,
            **({"layers_limit": gen_layers_limit} if gen_layers_limit is not None else {}),
        )
        if out.loss is None:
            return torch.zeros((), device=device, requires_grad=True)
        return out.loss

    def forward_gen3_single_response(question: str, document: str, response_str: str) -> torch.Tensor:
        ...
        attn_mask = torch.cat([torch.ones(1, past_len, device=device, dtype=torch.long),
                               torch.ones(1, La,       device=device, dtype=torch.long)], dim=1)
        gen_layers_limit = int(prefill_layers) if (args.also_cut_gen and int(prefill_layers) > 0 and int(prefill_layers) < int(n_layers)) else None
        out = model(
            input_ids=input_ids,
            past_key_values=lower_cache,
    ...
    def forward_two_stage_original_batched_responses(question: str, document: str, responses: List[str]) -> torch.Tensor:
        ...
        try:
            ...
            # Fallback: compute per response and average
            loss_sum = 0.0
            valid = 0
            for bi, a_ids in enumerate(ass_ids_list):
                La = a_ids.size(0)
            ...
            if valid == 0:
                return torch.tensor(0.0, device=device, requires_grad=True)
            return torch.tensor(loss_sum / valid, device=device, requires_grad=True)
    ...
    for epoch in range(1, args.epochs + 1):
            ...
            for i in range(len(qs)):
                ...
                if args.gen3:
                    if use_batch and isinstance(rlists[i], list) and len(rlists[i]) > 0:
                        ...
                        loss = forward_gen3_single_response(qs[i], ds[i], rstrs[i])
                else:
                    if use_batch and isinstance(rlists[i], list) and len(rlists[i]) > 0:
                ...
                # Extra safety: guard against non-finite gradients
                grad_ok = True
                ...
                wandb.log({
                    ...
                    "lr": current_lr,
                    "step": global_step,
                    "train/prefill_layers": int(prefill_layers),
                })
        ...
        if accelerator.is_main_process and val_avg < min_val_loss:
            ...
            # 6) Remote code: copy modeling base + wrapper (per gen) + pxgenerate helpers
            try:
                # Base modeling file always copied
                if IS_MISTRAL:
                    base_src = Path(__file__).parent / "modeling_mistral_partial.py"
                    base_dst = best_dir / "modeling_mistral_partial.py"
                    wrapper_src = (
                        Path(__file__).parent / ("modeling_latentrag_mistral_gen3.py" if args.gen3 else "modeling_latentrag_mistral_gen2.py")
                    )
                    wrapper_dst = best_dir / wrapper_src.name
                else:
                    base_src = Path(__file__).parent / "modeling_partial_layer.py"
                    base_dst = best_dir / "modeling_partial_layer.py"
                    wrapper_src = (
                        Path(__file__).parent / ("modeling_latentrag_llama_gen3.py" if args.gen3 else "modeling_latentrag_llama_gen2.py")
                    )
                    wrapper_dst = best_dir / wrapper_src.name
                ...
                # pxgenerate helper (used by Gen2 wrappers)
                px_src = Path(__file__).parent / "evaluate" / "utils" / "partial_xgenerate.py"
                if px_src.is_file():
                    shutil.copy2(px_src, best_dir / "partial_xgenerate.py")

                print(f"[Best] Injected base+wrapper remote-code: {base_dst.name}, {wrapper_dst.name}")
            except Exception as e:
            ...
            try:
                ...
                # Point to the wrapper module we just copied
                if IS_MISTRAL:
                    auto_map_val = (
                        "modeling_latentrag_mistral_gen3.MistralForCausalLM" if args.gen3
                        else "modeling_latentrag_mistral_gen2.MistralForCausalLM"
                    )
                else:
                    auto_map_val = (
                        "modeling_latentrag_llama_gen3.LlamaForCausalLM" if args.gen3
                        else "modeling_latentrag_llama_gen2.LlamaForCausalLM"
                    )
